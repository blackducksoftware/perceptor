...
func TestFetchCurrentVersion(t *testing.T) {
	...
	currentVersion, err := client.CurrentVersion()
	if err != nil {
		t.Errorf("unable to get current version: %s", err.Error())
		return
	}

	log.Infof("successfully fetched current version %+v", currentVersion)
}
