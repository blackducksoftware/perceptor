...
function main() {
  ...
  if [ $? -eq 0 ]; then
    if [ $do_scan -eq 1 ]; then
      ...
      "${THISDIR}"/scan.cli.sh $hub_parameters --insecure --username "$hub_user" $dry_run --project "$hub_project" --release "$hub_version" --name "$name_arg" $optional_args "${IMAGE_TARFILE}"
      cmd_status=$?
    ...
    if [ $do_inspect -eq 1 ]; then
      ...
      "${INSPECTOR_SHELL_SCRIPT}" --hub.always.trust.cert=true --hub.username="$hub_user" --hub.url=$HUB_URL --hub.project.name="$hub_project" --hub.project.version="$hub_version" "${IMAGE_TARFILE}"
      cmd_status=$?
      if [ $cmd_status -ne 0 ]; then
        exit $cmd_status
      fi
